...
| UX model | Turn-based click start/stop (delimits a turn; audio streams within it). **Phase I — optional auto-VAD added (additive; manual click-start/stop PERSISTS as the default + fallback): realtime server-VAD via the FE's `turn_detection: server_vad` in `session.update` (063-s1), cascade auto-finalize on Deepgram's utterance-end (062). Both ship single-utterance auto-VAD first; per-utterance/continuous multi-turn is a follow-up.** | Reliable, measurable; manual is the default — auto-VAD is additive for cascade-vs-realtime streaming parity (relaxes the original "no VAD tuning" in Phase I) | Hold-to-talk later |
...
export type TurnViewModel = {
  ...
  werWer?: number;
  outputAudioTokens?: number;      // (092) realtime-only — EXACT output audio tokens from response.done.usage; honest-omit when absent
  errors: UiError[];
};
...
> **(§39/097/099 realization — empty-silence handling, two sides)** An auto-VAD/continuous turn can finalize as `Completed` with **0 transcripts** (the loop re-armed and caught no speech). Both the summary (backend) and the per-turn display (frontend) must skip these — but a **cost-bearing** 0-transcript turn (every realtime turn — realtime transcripts are FE-store-side, never persisted on the turn, yet the turn carries a real cost) is REAL evidence, not silence. So the empty-silence predicate is **`Completed && 0 transcripts && cost == null`** (AND, not OR) on BOTH sides: the backend `SessionSummaryService.IsEmptySilence` excludes it from `ModeSummary` (097, refining §39/083), and the frontend `selectDisplayTurn` (099) — the FE-display sibling — skips it so the per-turn MetricsPanel/CostPanel show the last MEANINGFUL turn rather than blanking to n/a. The FE per-turn panels read `selectDisplayTurn(state)` (NOT a raw `currentTurn ?? turns[last]`, which lands on the spurious silence turn); the session-average rows + `ComparisonSummary` read `state.summary` (the already-correct backend summary), so they were never affected.
...
**`POST /api/sessions/{sessionId}/mode`** — switch the session's interpretation mode mid-session (Finding 2c). Body `SetModeRequest { mode }` (the target mode); returns the updated `InterpretationSession`.

> **(050 realization — mode-switch endpoint + Flow-G wiring)** `SessionsController` → `SessionService.SwitchMode` → `SessionStore.SwitchMode` (atomic **re-entrant**: under one gate it calls the now-wired `RecordModeTransition` then swaps `config.CurrentMode` — no torn window for a concurrent `CreateTurn`). The request DTO is a **raw `string? Mode`** (not the enum) so an off-enum/blank value rejects at the service chokepoint as a **sanitized `400 session.invalid_mode`** (lesson §27 reuse — an enum-typed DTO would throw in the deserializer → framework ProblemDetails, not our `UiError`); unknown id → `404 session.not_found`. `config.CurrentMode` is thus **mutable mid-session** (was effectively frozen post-create) → `CreateTurn` stamps the switched mode (the 2c fix — by-mode comparison stays valid). The server-derived `ModeTransitionEvent` (`fromMode`/`toMode`/`directionAtTransition`/`occurredAt`=`IClock.UtcNow`/`clockSource`=server/`triggeredByTurnId`=null) graduates the built-but-unwired **Flow-G** timeline (ARCH-017; feeds H.3). `/mode` does **not** persist (in-memory like `CreateTurn`) — the transition rides the next `/complete` (best-effort) / `/end` (MUST) write. _(An already-ended session is not guarded against `/mode` today — LOW single-user, frontend-gated; G-hardening.)_
...
### Dev-only API (Development environment; unmapped in Production)

**`POST /api/dev/tts`** (086 — G.4 soak-harness support) — `{ text, voice?, language? }` → raw synthesized audio bytes (via the integrated OpenAI TTS provider). **Development-environment-gated** — the route is mapped only when the host is `IsDevelopment()` and is **absent in Production** (so it is unreachable in a deployed build). It exists solely so the synthetic soak-harness can synthesize the scripted EN↔ES conversation audio once, cache it, and reuse it across both modes (fair/repeatable) by injecting it at the browser `getUserMedia` boundary. **Invariant #3:** stateless — touches no `SessionStore`/persistence and never persists the synthesized audio.
...
> **(098 realization, Finding B)** A live `gpt-realtime` repro IGNORED a mild interpreter prompt ("speak only the translation — no commentary"): it ANSWERED a spoken question, in the WRONG language, with meta-commentary. The shipped templates (`RealtimeClientSecretMapping.cs` — `DefaultInstructionsTemplate` one-direction + `DefaultBidirectionalInstructionsTemplate`; the mint is the single source, the FE sends no `instructions`) are RESTRUCTURED with four elements: (1) emphatic conduit framing ("You are ONLY a translation conduit. You are NEVER a conversational party."), (2) a **target-language lock** ("ALWAYS output ONLY the {target} translation … never any other language, never your own words"), (3) explicit question-handling ("translate the QUESTION itself into {target} — NEVER answer, respond to, explain, or add anything"), (4) an example. The instruction string + wiring is deterministic + test-pinned (clauses present in both renders, survive `{source}`/`{target}` substitution); the **effectiveness is eval-observed** via a live realtime confirm (acceptance "What is your name?" → "¿Cómo te llamas?", not answered; the mint must be restarted to take effect), never unit-asserted.
...
> **056-c1 cascade per-turn metrics-correctness realization (2026-05-31, real-key smoke).** A live smoke found the per-turn cascade metrics visibly wrong on a **completed** turn; `deriveTurnMetrics` got three ARCH-013 fixes (web §25). **(1) Per-stage STT/Translation/TTS now show DURATIONS** — `deriveTurnMetrics` **differences the stage markers** via absolute-timestamp `Between` (`stt.final`−`cascade.audio.received`, `translation.final`−`translation.started`, `tts.complete`−`tts.started`), keyed `stt`/`translation`/`tts` to match the panel; it previously passed the raw `{eventName: relativeMs}` map through, whose keys never matched the panel → a **permanent `n/a`** on a turn whose events were all present. Absent/negative stage durations are **omitted** (honest `n/a`, never a fabricated 0). **(2) Cascade speech-end re-anchored to `stt.final`** for `speech_end_to_first_audio_ms` (responsiveness): pre-VAD there is no true speech-end signal, and the **manual** `turn.recording.stopped` is held seconds after speech → `first_audio − recording.stopped` went **negative**. Deepgram endpointing (`stt.final`) ≈ true speech-end, so cascade responsiveness now anchors on `stt.final` (falls back to `recording.stopped` when absent; realtime unchanged). **`speech_end_to_playback_ms` keeps the literal `recording.stopped` anchor** (backend-consistent — the `MetricsAggregator`/`ModeSummary` average uses `recording.stopped`). **(3) The `<target` badge sits on the RESPONSIVENESS headline** (both modes — `<3s`/`<1.5s` are responsiveness targets, **never** total-turn, which includes talk+hold time); total-turn is a secondary row with **no badge**; `latencyTier` returns `na` for a negative value (the value stays disclosed, only the badge mutes). **⚠️ Three residuals are BACKEND, not frontend (flagged as backend follow-ups, → the cascade-cost-backend queue):** **(a)** a **per-turn / session-avg anchor divergence** — the per-turn cascade responsiveness now anchors `stt.final`, but the backend `ModeSummary.avgSpeechEndToFirstAudioMs` (the session average of the *same* metric) still anchors `recording.stopped` → re-anchor `SessionSummaryService` (cascade) to `stt.final`, or explicitly document the per-turn(responsiveness)/session-avg(literal) distinction so the two reconcile; **(b)** the cost `n/a` on a successful cascade turn (the C.2 live `usage`-nesting); **(c)** the **coincident `tts.started`==`tts.first_audio`** stamp (the backend emits both at the same instant → a `0 ms` TTS-first-audio). The pre-VAD speech-end honesty limit is fully resolved only by **Phase I** server-VAD.
...
> **(094 realization)** When the exact-token path is used (`response.done.usage` audio-token counts present), **cached input-audio is priced at the cached rate, REMOVED from the full-rate base** — not priced at the full rate and then added at the cached rate on top (the prior over-count: the cached-audio slice was billed full+cached, ~80x over → a ~1.5x per-turn over-count on cached-heavy realtime turns, which are common mid/late-session because the model re-bills the accumulating conversation context as input each turn; surfaced by the 2026-06-01 soak). `CompleteTurnRequest.cachedAudioInputTokens` / `CostUsage.CachedAudioInputTokens` carry the cached-audio **SUBSET** (`input_token_details.cached_tokens_details.audio_tokens`, clamped <= the input-audio total), not the aggregate `cached_tokens`. A model with no configured cached rate (`gpt-realtime-mini`) bills cached audio at the full input rate (net unchanged from a no-cache world). FE forwards the subset (095); BE re-prices (094).

> **(097 realization)** Realtime turns persist with **0 transcripts** (realtime source/target transcripts accumulate FE-store-side and are not sent on `/complete`) but carry a real `CostEstimate`. `SessionSummaryService.SummarizeMode`'s `IsEmptySilence` filter (added at 083/§39 to drop phantom cascade auto-VAD silence turns) therefore requires **`CostEstimate is null`** in addition to `Completed && 0-transcripts` — otherwise it excluded every realtime turn from the per-mode `ModeSummary`, blanking the realtime Cost/min in the live UI (Finding A). A cost-bearing turn produced billable work → it is real evidence, never phantom silence (cost-based discriminator). _(Whether realtime transcripts SHOULD be persisted on the turn — a new `/complete` wire-path so the history drill-in shows them — is a tracked future decision, not done here.)_
...
**Flow D — WER (push-to-talk):** open Evaluation panel → select phrase → **click Record → a 3·2·1 "get ready" countdown → a visible "Recording" state → read aloud at your own pace → click Stop** → `transcribe` → `wer` → persist (096; an empty/no-speech capture degrades to "No speech detected — n/a", never a fabricated score).
...
> **(096 realization, Finding 3 — ARCH-017/015)** Flow D is **push-to-talk**, not a silent fixed-duration auto-record. A silent fixed 6 s window (no recording indicator / no "speak now" prompt) produced mathematically-correct-but-misleading WER (the user read late → the STT mis-heard → a high WER read as "fabricated"). The Evaluation panel is a small state machine — `idle → countdown → recording → evaluating → result` — where the user clicks Record (a short 3·2·1 lead-in → a visible "Recording — click Stop" state), reads at their own pace, and clicks Stop; a 60 s safety auto-stop backstops a forgotten recording. An empty/whitespace STT hypothesis returns a distinct **no-speech** outcome ("No speech detected — n/a") — never a scored empty hypothesis (all-deletions = a misleading 100%) — and creates **no** eval turn (no orphan / `WerSummary` pollution). The manual-stop capture seam is `audioCaptureController.startBlobRecording() → { stop }` (replacing the fixed-duration `recordBlob`); the panel owns the recording lifecycle via `evaluateFromBlob`.
...
> **(G.2b realization — `.env` auto-load at startup)** `DotEnvLoader.AutoLoad(AppContext.BaseDirectory)` runs in `Program.cs` **before** `WebApplication.CreateBuilder` (the config providers snapshot the environment there, so the vars must exist first → they then flow through the env→section bridge above). It walks up from the assembly base dir to the nearest ancestor repo-root `.env` and loads it **fill-gaps only** — an already-set process var (an explicit `export`, CI, or prod) WINS; `.env` is only the fallback for absent keys. **Degrades silently when absent** (prod/CI; lesson §3 — same posture as the pricing/phrase file-loaders). **⭐ Gated to the real API entry point** (`ShouldAutoLoad`: process entry assembly == `AiInterpreter.Api`; the VSTest host / test assembly / null all SKIP) so the `WebApplicationFactory` test host never loads the dev `.env` — its real keys would flip provider DI from fakes to live (real network calls / a non-deterministic suite, ARCH-020). Keys stay process-side (invariant #1 — the loader never logs/echoes a value; `.env` stays gitignored). Net: a clean clone now runs with plain `dotnet run` (no manual `set -a && source ../../.env`). (G.2b.)

<a id="arch-029"></a>

### Build & Run Contract

Backend listen URL/port (e.g. `http://localhost:5179`); frontend dev port (e.g. `http://localhost:5173`); `VITE_API_BASE_URL` + a Vite dev proxy (or CORS allow-origin) so the SPA reaches the API and the cascade WebSocket; **secure-context note** (localhost is secure; non-localhost needs HTTPS for `getUserMedia`/WebRTC); `scripts/` for one-command start where helpful. The backend **auto-loads the repo-root `.env`** at startup (G.2b — see the env→section bridge's G.2b realization in ARCH-028), so a clean clone runs with plain `dotnet run` (no manual env sourcing; fill-gaps so an explicit `export`/CI/prod still wins). (Demo script lives in ARCH-021.)
...
> **Realization note (G.4, 086–093 / 2026-06-01).** The 5-minute run is now ALSO driven by an automated **synthetic soak-harness** (dev-only `?soak=1`; manual eyeball remains the fallback). It injects pre-synthesized OpenAI-TTS audio at the **`getUserMedia` boundary** (a DI seam on `audioCaptureController`, cascade parity with the realtime client's seam) — so the real worklet PCM encode + real WebRTC track-send run unchanged, measuring the GENUINE pipeline (clean-separation preserved, ARCH-007: capture-boundary injection + store-read, never bypassing the store). It drives a scripted 5-min bidirectional EN↔ES conversation per mode and produces a structured `SoakReport`: **no-disconnect** = a precise count of the two transport-close codes (`cascade.connection_lost` / `realtime.session.disconnected`), excluding provider-error failures; **no-drift** = a one-sided per-turn latency-slope (accumulating-lag only) + overlap detection (realtime overlap-duration token-derived from `response.done.usage`, cascade char-estimate — disclosed per-mode via `overlapBasis`; `overlapMeasured` when an output-duration signal exists); **no-leak** = a warm-up-then-trend `performance.memory` heap verdict; plus latency, cost, and **WER-via-script** (scored against the committed script through the canonical `/wer` explicit-reference path). The deterministic core is unit-TDD'd (the FE analogue of the §18/§20 backend pattern, web §35); the browser-audio + real-provider live drive is manual-smoke. The full real-audio 5-min × both-modes run is the **manual real-key run** (its `SoakReport`s feed the G.5 write-up).
...
| `ISttProvider` + `SttEvent`/`SttRequest`/`AudioFrame` | ARCH-012 | streaming STT contract; **`SttEvent` gained `SttUtteranceEnd`** (Phase I 062 — a no-text endpointing marker mapped from Deepgram's `UtteranceEnd`, driving cascade auto-finalize; manual mode ignores it) |
...
| `UiSessionState` / `TurnViewModel` / `UiError` (TS) | ARCH-007 | frontend projections; **`TurnViewModel`** +`outputAudioTokens?: number` (092 — realtime-only; the EXACT `response.done.usage` output-audio-token count surfaced on the turn, honest-omit when absent) |
| API DTOs: CreateSession, EndSessionResponse, CreateTurn(Response), CompleteTurn(Request/Response), SetModeRequest, AppendEvents, ClientSecret, CascadeStream msgs, Transcribe, Wer, ConfigResponse | ARCH-009 | camelCase serializations of the above. **Cascade WS (C.4a):** client `start{sessionId,turnId,direction,encoding,sampleRate,translationModel,ttsVoice}` + `stop{}`; server `transcript{segment}`/`latency{event}`/`audio{contentType,seq,base64}`/`cost{estimate}`/`error{error}`/`done{turnId,status}` (the wire contract; `CascadeOutputEvent`/`CascadeStartParams` stay **internal** — only these WS DTOs serialize). `CreateSession` (request) → returns the full `InterpretationSession`; `EndSessionResponse` (B.9c-i) = `{ session, persistedPath (filename-only), persistenceWarning? }` at 200; `CreateTurnResponse` = `{ turnId }`; `CompleteTurnRequest` (B.9c-ii; realtime-only; **E.2b** +`outputAudioDurationMs?`; **053-C2a** +`inputAudioTokens?`/`outputAudioTokens?`/`cachedAudioInputTokens?`) = `{ audioDurationMs?, outputAudioDurationMs?, transcripts?, status?, inputAudioTokens?, outputAudioTokens?, cachedAudioInputTokens? }` (cost/WER/model/voice are NOT client-supplied; the `*AudioTokens?` fields carry the realtime turn's EXACT DC `response.done.usage` audio-token counts → `EstimateRealtime` prices from them exactly when present, else the seconds×50 estimate — supersedes the `outputAudioDurationMs?` played-duration approach; the frontend extract-`response.done.usage`+POST + the TS mirror is **053-C2b**; **094/095:** `cachedAudioInputTokens` carries the cached-AUDIO subset `cached_tokens_details.audio_tokens`, priced at the cached rate REMOVED from the full-rate base — not the aggregate `cached_tokens`); `CompleteTurnResponse` = `{ turn, persistenceWarning? }`. **Cascade blob fallback (C.5):** `POST /api/cascade/turn` (multipart) → `CascadeTurnResponse` = `{ turn, audioBase64?, audioContentType?, persistenceWarning? }` (the assembled `InterpretationTurn` + the concatenated target TTS audio in-body, **response-only — never persisted**, invariant #3); the multipart form binds the turn params (`CascadeTurnForm` — input-binding only, not serialized out); `CascadeBlobParams`/`CascadeBlobResult` stay **internal**. **Realtime token (E.1):** the `ClientSecret` DTO = `RealtimeTokenRequest` `{ sessionId, direction, model? }` → `RealtimeTokenResponse` `{ clientSecret (`ek_…`), expiresAt (ISO-8601), model }`; standard key is server-side-only (invariant #1, Bearer-only), the `ek_` is response-only/never-persisted (invariant #2), GA top-level→camelCase mapping per ARCH-010; `RealtimeMintOutcome`/`RealtimeModelCatalog` stay **internal**. **Evaluation (F.1a; +090 explicit-reference):** `WerRequest` `{ sessionId, turnId?, phraseId?, hypothesis, reference? }` → `WerResponse` `{ result: WerResult, persistenceWarning?: UiError }` — **090** added the optional `reference` (an explicit reference string that wins over the store lookup) and made `phraseId` optional (one of `phraseId`/`reference` must resolve a reference); used by the G.4 soak-harness (the computed `WerResult`; `persistenceWarning` set iff a `turnId`-attach write degraded); id fields `[MaxLength(256)]`, `hypothesis` capped in-service (not `[MaxLength]`) → `400 evaluation.invalid_phrase`; unknown phrase → `404 evaluation.phrase_not_found`. `EvaluationWerStatus`/`EvaluationWerOutcome` stay **internal**. **Evaluation transcribe (F.1b):** `POST /api/evaluation/transcribe` (multipart) binds `TranscribeForm` `{ sessionId, phraseId, language, audio, timestamps? }` (input-only, not serialized) → `TranscribeResponse` `{ hypothesis, sttProvider, sttModel, latencyEvents }`; STT-only, stateless (no turn/persist, invariant #3); upload size+type-validated via the shared `CascadeUploadValidation` (`413`/`415`, invariant #5); id over-cap → `400 evaluation.invalid_request`. `EvaluationTranscribeStatus`/`EvaluationTranscribeOutcome` stay **internal**. **Mode switch (050):** `SetModeRequest` = `{ mode }` (raw `string? Mode` — validated in-service → sanitized `400 session.invalid_mode`, not `[MaxLength]`/enum) → returns the updated `InterpretationSession`; the endpoint records a server-derived `ModeTransitionEvent` (Flow-G; `RecordModeTransition` now wired). `SwitchModeOutcome`/`SwitchModeStatus` stay **internal**. **Session list (H.3-backend 065):** `GET /api/sessions` → a bare `SessionListItem[]` JSON array where `SessionListItem` = `{ sessionId, label?, startedAt, endedAt?, turnCount, modes[] }` — a lightweight summary projected from the persisted sessions on disk (most-recent-first via `SessionPersistenceReader`, Option B — the first request-reachable disk-read tier, ARCH-016); a missing dir → empty; a misconfig (file-where-dir) → sanitized `sessions.read_failed` (ARCH-018); `SessionPersistenceReader`/`SESSION_MAX_READ_BYTES` (ARCH-028) stay internal/config; the full session is the existing `GET /{id}` (its disk-read fallback is a follow-up). **Cascade WS `start` (Phase I 062):** gains `autoVad?: bool` (`CascadeStartParams.AutoVad`; true ⇒ the orchestrator auto-finalizes the turn on Deepgram's utterance-end via the `SttUtteranceEnd` provider event, manual-stop still a valid early-terminal). **Realtime turn-detection (Phase I, FE-controlled):** the realtime `session.update` `turn_detection` switches `null`(manual)↔`{type:'server_vad',…}`(auto) per the FE `turnControlMode` — no broker change. **Dev-only (086):** `POST /api/dev/tts` `{ text, voice?, language? }` → raw synthesized audio bytes, **Development-environment-gated / unmapped in Production**, stateless (invariant #3) — G.4 soak-harness audio synthesis only. |
