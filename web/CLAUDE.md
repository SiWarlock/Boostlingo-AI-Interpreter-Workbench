...
| _Deferred-opaque (intentional, pragmatic-accrete):_ `SessionSummary`, wire `InterpretationTurn` = `Record<string, unknown>`; `modeTransitions: unknown[]` | ARCH-005 / ARCH-009 | Typed opaquely until a slice consumes them — `SessionSummary` tightens at F.3 (comparison summary); wire `InterpretationTurn` may stay opaque (the frontend builds `TurnViewModel`, not the raw wire turn). The 4 streaming wire types above graduated out of this row at D.4a. (D.1) |
