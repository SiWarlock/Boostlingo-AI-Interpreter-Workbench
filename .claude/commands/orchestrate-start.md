...
You're starting an orchestrator session for AI Interpreter Workbench. Orient before taking any action.
...
(b) **The active area's cross-doc invariants table has changed** since the prior brief touched it (check via `git log --oneline <area>/CLAUDE.md`).
