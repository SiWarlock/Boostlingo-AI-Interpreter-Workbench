...
**PHASE C COMPLETE — the full backend cascade ships (streaming WS + blob fallback), hardened + tested vs fakes.** ✅ Phases A + B + C done. ⏭ **Next: Phase D (frontend, `web/`)** — a FRESH frontend team (orchestrator + `web/` implementer) cycles in per the user's direction; both backend teammates cycled out at this round-seal.

- **Round sealed 2026-05-29 (C.4b + C.5 — Phase-C close; user-directed backend round-seal + Phase-D transition)** — round-seal: this `docs(tasks)` commit at HEAD · code commits this round: C.4b `b226fe9`+`4165551`, C.5 `88bac2b`+`49ec08d` · session doc: `005-2026-05-29-phase-c-c4b-c5.md` (`1944d02`) · next-slice target: **D.1**.

**Last landed:** Phase C close (this round): **C.4b** WS-boundary hardening — `b226fe9` (robustness: fail-closed streams, idempotent terminal completion, start-frame field validation, ContentType clamp, `ResolveVoice`, cost-fold) + `4165551` (SECURITY: bounded `Channel<AudioFrame>` + `Origin` validation); **C.5** blob fallback — `88bac2b` (SECURITY: upload validation, invariant #5) + `49ec08d` (`POST /api/cascade/turn`, a thin adapter over the **unchanged** streaming orchestrator). **Both cascade transports** (`WS /api/cascade/stream` streaming + `POST /api/cascade/turn` blob) are wired, hardened, persist, and tested vs fakes. **Suite 211→284 green** (build 0W/0E; format clean; security-reviewer invariants #1–#5 PASS on both safety slices). Lessons §1–§23 banked (§22–§23 this round). Push deferred (no remote).
...
_(Triaged at the 2026-05-29 Phase-C-close (`C.4b`+`C.5`) `/orchestrate-end`: **1 deleted** — the B.5 cost-evidence-trail polish (last-consumer C.5 reached; resolved at C.5 — (a) `cachedAudioInputSeconds`-always-0 + (b) `EstimateTranslation` null→0 decided as intentional MVP behavior, and the deeper blob-STT-cost is now a documented ARCH-014/G.5 limitation + the tracked FORK-1a enhancement); **1 added** — `CascadeTurnForm` `[Required]` direction (origin C.5, D-adjacent); **1 updated** — the B.9c-ii per-item-string-caps item (the WS + blob client strings are now capped at 256 in C.4b/C.5; the B.9 HTTP-DTO item caps remain). **Informational:** 6 kept — but all are spread to D/E/G with last-consumer markers (legitimately phase-distributed, not monotonic accumulation). Prior C.4a-close triage: 1 inlined (double-End→C.4b, now landed) + 2 deleted.)_
...
- **`CascadeTurnForm` `[Required]` on Source/Target (origin: 2026-05-29 C.5, D-adjacent)** — the blob endpoint's multipart `CascadeTurnForm` lacks `[Required]` on the `source`/`target` direction fields, so a missing direction silently binds `En→En`. Bounded for the no-auth MVP; **becomes live when the Phase-D frontend POSTs to `/api/cascade/turn`** — the frontend cascade client should always send direction, and the endpoint should `[Required]`-guard it (a fuller form-validation pass). _(last-consumer-slice: D — the frontend cascade/blob client; fold into a D form-validation slice or a hardening pass.)_
