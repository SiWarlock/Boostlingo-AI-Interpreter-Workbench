...
**PHASE E (Realtime) — BACKEND COMPLETE; ⏭ frontend (E.3) next.** ✅ Phases A–D + the Phase-E **backend** (E.1 broker + E.2 ingest [B.9c-ii] + E.2b realtime cost + E.5-backend stale-flush) done; **backend suite 316 green**. ⏭ **Next: E.3 — browser WebRTC client** (frontend `web/`). Lead-directed wholesale backend→frontend cycle at this seal: a fresh orchestrator + a `web/` implementer cycle in.
...
_(Triaged at the 2026-05-30 Phase-E-backend-close `/orchestrate-end`: **1 deleted, 0 deferred, 0 inlined; 2 updated, 6 added (E.1/E.2b/E.5-backend origins), rest SPREAD.** **DELETED (1):** D.6 metrics-sourcing — completed in D.6 (session 007). **UPDATED (2):** capture-reuse-for-realtime last-consumer E.1/E.2 → **E.3** (the backend E.1/E.2 shipped, but the capture-surface decision belongs to the frontend WebRTC client); realtime turn cost+errors — the **cost half is DISCHARGED at E.2b**, the failed-turn `Errors` half remains → last-consumer E.5; B.9c-i model-allowlist realtime-half discharged at E.1. **ADDED (6):** RealtimeModelCatalog↔ConfigService unify (G); no-auth-gate on the mint endpoint (G); frontend reports `outputAudioDurationMs` (E.4, LOAD-BEARING for realtime cost accuracy); cached-audio-input seconds (F); TTL stale-sweep (G). **SPREAD:** the rest carry valid last-consumer markers (E.3/E.4/E.5/F/G) — phase-distributed, not monotonic. **Next brief: E.3 (frontend WebRTC client)** — fresh `web/` impl; the KEEP for E.3 is capture-reuse-for-realtime. Prior Phase-D-D.1–D.5 triage superseded.)_
...
- **Capture reuse for the realtime path (origin: 2026-05-29 D.3, → E)** — D.3's `audioCaptureController.onFrame` emits **linear16 frames** (cascade-shaped, for the WS). Realtime (WebRTC) doesn't consume frames — it `pc.addTrack`s the raw mic track. So E's capture reuse is a different surface: either extend the controller to expose the raw `MediaStream`, or E does its own `getUserMedia`. Clean-separation already holds (the controller imports no store/transport), so the decoupling is sound; this is just a "decide E's capture surface" note. _(last-consumer-slice: **E.3** — the realtime WebRTC client; E.1/E.2 backend shipped but did NOT consume this — it's the frontend capture-surface decision. **KEEP — fold into the E.3 brief.**)_- **Cascade `playback.started` not persisted (origin: 2026-05-29 D.5, → F.3 / G.5)** — `playback.started` is a **frontend-only display marker** for cascade: the cascade WS has no client→server latency-report channel (that's realtime's `POST …/events`, E.4), and the turn is persisted by the WS on `done` *before* playback starts. So the persisted/aggregated cascade summary shows speechEnd→playback **`n/a`** (honest degradation — "n/a for unavailable metrics, never an error"). **Default: accept + document (G.5 limitations list).** F.3 decides whether a cascade client→server latency channel is worth adding for the comparison write-up (affects the cross-mode latency comparison — a deliverable). _(last-consumer-slice: F.3 — the comparison summary; may escalate the add-a-channel decision.)_
...
- **Frontend store unbounded growth — `latencyEvents[]` per-turn + `turns[]` per-session (origin: 2026-05-29 D.6, → G.4 — LOW)** — D.6 retains the raw `latencyEvents[]` per turn + `turns[]` accumulates per session; both grow unbounded over a long session. Single-trusted-user (ARCH-019); realistic bound is the turn/session lifecycle; mirrors the backend's own bounded-channel deferral. _(last-consumer-slice: G.4 — the 5-min no-leak preflight.)_
...
- **Realtime turn errors population (origin: 2026-05-29 B.9c-ii, → E.5) — COST HALF DISCHARGED at E.2b** — the **`CostEstimate` half is DONE** (E.2b `a6bc540` wired `CostEstimator.EstimateRealtime` into `/complete`). **Still open:** a **failed realtime turn's `Errors` population** is E's frontend job (E.5 — the WebRTC client maps `realtime.session.disconnected`/`response.error` → a `ProviderError` reported on the turn). The summary already handles null cost + empty errors. _(last-consumer-slice: E.5 — the realtime connection-lifecycle + error surfacing.)_
...
> **Backend-status note (2026-05-30):** **Flow-G mode-transition recording is already built** (`SessionStore.RecordModeTransition`). **Flow-H backend stale-session flush ✅ SHIPPED** (E.5-backend, brief 037 — on-`Create` auto-end+persist of all un-ended priors via `CreateAsync`, reusing `EndAsync`; owed write uses `CancellationToken.None`; 311→316 green). The rest of E.5 (frontend WebRTC lifecycle — `RTCPeerConnection` teardown, ICE-disconnect surfacing, `expiresAt` re-mint, the frontend Flow-G/H wiring) **stays here for the frontend impl**.
- [ ] One persistent `RTCPeerConnection` across turns (60-min cap noted); teardown on End/mode-switch (stop tracks, close DC, close pc, release stream). On ICE failed/disconnected → persist `realtime.session.disconnected` + advise switch-to-Cascade. Track `expiresAt` → re-mint before expiry. **Flow G** (mode switch between turns; emit `ModeTransitionEvent` — *backend `RecordModeTransition` built*) + **Flow H** (refresh recovery; backend stale-session flush — *backend portion = the E.5-backend pull-forward slice*).
