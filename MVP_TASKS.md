...
**PHASE B COMPLETE (B.1–B.10).** Next: **Phase C — Cascade real providers + streaming transport** (highest-risk integration #1). **First slice target: C.1 (`DeepgramSttProvider` — live WS + pre-recorded fallback).** ⏸ **Team cycled at the Phase-B boundary (2026-05-29); a FRESH team resumes at C.1.**

- **Team cycled 2026-05-29 (Phase-B close, planned)** — round-seal: this `docs(tasks)` commit at HEAD · last code commit: `759cac2` (B.10) · session doc: `003-2026-05-29-phase-b-b7-b10.md` (`30e2ba7`) · next-slice target: **C.1**.

**Last landed:** Phase B B.7a–B.10 (this round): B.7a store+writer+sentinel (`9bfea76`), B.7b summary (`2d6a591`), B.8 sanitizer+UiError (`0262ce1`), B.9a global handler (`88a7f3e`), B.9b config (`278caf2`), B.9c-i session endpoints (`e39ec7b`), B.9c-ii turn endpoints (`9e0066b`), B.10 provider-boundary suite (`759cac2`). **Phase A + Phase B COMPLETE.** **161 tests green** (verified full `dotnet test`), runnable host on `:5179`, full session/turn/config HTTP surface + global sanitizing handler wired. **Commit cadence:** commit-as-we-go on `main` per logical unit (push deferred; no remote configured). Lessons §1–§17 banked.

**Open safety/handoff items for the fresh Phase-C team** (full detail in session doc `003` + the Carry-forward section + lessons §11–§17):
- **C.1–C.3 real providers:** implement Deepgram STT / OpenAI translation+TTS behind the **unchanged** B.1 interfaces; exception/HTTP → `ProviderError` per the ARCH-012 mapping table (the B.10 `ProviderBoundaryTests` is where real-provider conformance + real-provider cancellation land, "extended").
- **C.4 (security MEDIUM, precondition Findings):** WS `start` `encoding` allowlist before building `CascadeStartParams` (content-type header-injection surface); `TtsFirstAudio.ContentType` clamp (low); `Overall` `LatencyStage` enum member + `turn.recording.*` re-stamp; stream-without-terminal `<stage>.unknown` hardening; WS `Origin` validation (WS bypasses CORS); **double-End/complete guards** (now reachable once the WS + HTTP end-paths collide).
...
- **Build-confirm (non-blocking):** `gpt-5.4-mini` pricing rates (still `0.0`); `RealtimeTokensPerAudioSecond = 50` realtime factor — confirm against real billing in C/E.
- See **Carry-forward** for the hardening backlog (per-item string caps, model-allowlist, cost-estimate evidence-trail polish, realtime cost→E) and **D.1** (TS mirror types + Vite dev-config, re-sequenced).
...
_(Triaged at the 2026-05-29 Phase-B-close `/orchestrate-end`: 3 resolved items removed — Persistence `Result.Error` non-forwarding (satisfied by B.9's `SanitizeResult`/`ToPersistenceOutcome` — `Result.Error` never echoed); `SessionStore.Get` snapshot-vs-live (resolved — live-view accepted for the single-node MVP; residual concurrency tracked by the double-End/complete item); 404/405 `UiError`-vs-ProblemDetails (resolved B.9c-i — controller 404s are sanitized `UiError`, framework 404/405 stay ProblemDetails for unrouted paths). The 5 below are future-phase, last-consumer-marked.)_

- **Cost-estimate evidence-trail polish (origin: 2026-05-28 B.5)** — when C.4 wires real provider usage: (a) `CostEstimate.Units` always writes `cachedAudioInputSeconds` (0 when unsupplied) → consider omitting the key when null so the persisted trail distinguishes "no cache" from "0s"; (b) `EstimateTranslation` substitutes 0 for null tokens (silent $0) vs STT's degrade-on-null → align or document. _(last-consumer-slice: C.4.)_
- **Stronger model validation — allowlist (origin: 2026-05-28 B.9c-i, security follow-up)** — `POST /api/sessions` length-caps `RealtimeModel`/`TranslationModel` (`[MaxLength]`, mitigates the unbounded-string HIGH), but does not yet **allowlist** them against the published catalog (the two realtime + two translation models in `ConfigService`). Stronger follow-up: reject off-catalog model names at the create boundary (reuse the ConfigService catalogs). _(last-consumer-slice: C.4 or a hardening pass; client selectors already constrain to the catalog.)_
- **Double-`End` / double-`complete` races (origin: 2026-05-28 B.9c-i/B.9c-ii, LOW)** — `SessionStore.End`/`/end` summarize+persist + `/complete`'s read-modify-persist are unguarded against a concurrent second call (or a C.4 WS path colliding with an HTTP call) — last-writer-wins, documented in code. Single-node MVP tolerates it. Guard when C.4 makes concurrent end/complete paths reachable. _(last-consumer-slice: C.4.)_
- **Per-item request-string caps (origin: 2026-05-29 B.9c-ii, security MEDIUM — bounded)** — `[MaxLength]` caps collection *counts* (events/transcripts ≤500) but not per-*item* string lengths (`LatencyEvent.Name`/`Metadata` values, `TranscriptSegment.Text`). Bounded today by Kestrel's 30MB body limit + the count caps; full per-item caps need a design call (request-specific DTOs vs adding validation attrs to the ARCH-005 **domain** records — prefer request DTOs; don't pollute the domain contract). Low material impact (single-trusted-user workbench, ARCH-019). _(last-consumer-slice: a hardening pass / C.4.)_
- **Realtime turn cost + errors population (origin: 2026-05-29 B.9c-ii, → E)** — `POST …/complete` (realtime finalize) carries `audioDurationMs?/transcripts?/status?` only; a realtime turn's `CostEstimate` is **null until E wires `CostEstimator.EstimateRealtime` from reported usage** (cost is backend-owned, ARCH-014), and a failed turn's `Errors` population is E's (E.5 `realtime.session.disconnected`). The summary already handles null cost. _(last-consumer-slice: E.4/E.5.)_
