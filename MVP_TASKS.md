...
**PHASE D COMPLETE — the frontend cascade path ships end-to-end (D.1–D.7).** ✅ Phases A+B+C (backend) + Phase D (frontend) done. ⏭ **Next: Phase E (Realtime mode)** — highest-risk integration #2; needs real OpenAI Realtime keys; launch direction being confirmed with the user. Both teammates cycle out (wholesale) at this round-seal; the lead re-spawns a fresh team for E.
...
_(Triaged at the 2026-05-29 Phase-D-D.1–D.5-close `/orchestrate-end`: **0 deleted, 0 deferred, 0 inlined; 1 KEEP (D.6), 3 added, 2 updated, rest SPREAD.** **KEEP for the immediately-next brief (D.6):** the **D.6 metrics-sourcing design question** (added this round) — the D.6 brief must fold it in. **Added (3):** D.6 metrics-sourcing (→D.6, load-bearing); cascade `playback.started` not persisted (→F.3/G.5); blob-playback-fallback completeness (→G); SessionSetup `'ending'`-gate (→E.5); capture-reuse-for-realtime (→E.1/E.2) [the last two added across D.2/D.3]. **Updated (2):** the B.9c-i model-allowlist item (now client-side-mitigated at D.2 — selectors offer only catalog models; server-side allowlist + the onChange-cast runtime guard remain G); the C.5 `CascadeTurnForm [Required]` item (now client-side-mitigated at D.1 — `cascadeApi` always sends source+target; server `[Required]` guard remains). **SPREAD (informational):** all remaining items carry last-consumer markers (E.4/E.5, F.3, G) — legitimately phase-distributed, not monotonic accumulation; none completed/deferred this round. Prior Phase-C-close triage: 1 deleted + 1 added + 1 updated.)_

- **Stronger model validation — allowlist (origin: 2026-05-28 B.9c-i, security follow-up) — partially CLIENT-MITIGATED at D.2** — `POST /api/sessions` length-caps `RealtimeModel`/`TranslationModel` but does not **allowlist** them against the published catalog (`ConfigService`). **D.2 client-mitigation:** the model `<select>`s offer ONLY catalog models (`availableModels` reads the `ConfigResponse` catalog), so the frontend never *presents* an off-catalog choice. **Still open:** (a) a runtime membership guard on the D.2 `<select>` onChange `as RealtimeModel/TranslationModel` cast (defense-in-depth — a crafted DOM/event could still set an off-catalog value); (b) the **server-side** allowlist at the create boundary (reuse the ConfigService catalogs) — the authoritative guard. _(last-consumer-slice: a G hardening pass; bounded — backend length-caps + client selectors constrain today.)_
...
- **Capture reuse for the realtime path (origin: 2026-05-29 D.3, → E)** — D.3's `audioCaptureController.onFrame` emits **linear16 frames** (cascade-shaped, for the WS). Realtime (WebRTC) doesn't consume frames — it `pc.addTrack`s the raw mic track. So E's capture reuse is a different surface: either extend the controller to expose the raw `MediaStream`, or E does its own `getUserMedia`. Clean-separation already holds (the controller imports no store/transport), so the decoupling is sound; this is just a "decide E's capture surface" note. _(last-consumer-slice: E.1/E.2 — the realtime WebRTC client.)_
- **D.6 metrics-sourcing design question (origin: 2026-05-29 D.4a+D.5, → D.6 — LOAD-BEARING for D.6)** — the cascade WS streams per-stage `LatencyEvent`s; D.4a's `appendLatencyEvent` stores `stages[name]=relativeMs` only and leaves the **top-level deltas** (`speechEndToFirstAudioMs`/`…PlaybackMs`/`totalTurnMs`) undefined. D.6 must decide top-level sourcing: **prefer the backend's canonical metrics** (lesson §7/§12 — `relativeMs` is per-event display; the aggregator uses absolute `Timestamp`) over recomputing client-side. Specifically for **speechEnd→playback**: it must be computed as a **delta of absolute browser timestamps** (`playback.started.timestamp` − the speechEnd moment), NOT the D.5 `playback.started` `relativeMs:0` placeholder (rendering the placeholder as a latency would be a synthetic metric — streaming-honesty). This needs a **frontend `turn.recording.stopped` browser marker** stamped when `stop` is sent (NOT stamped in D.4b's `stopRecording`) — fold a tiny stamp into D.6 or a D.4b touch-up. _(last-consumer-slice: D.6 — the MetricsPanel; fold into the D.6 brief.)_
